...
function mirrorHalf(map: number[][], caller: Half): number[][] {
    ...
    switch (caller) {
        case Half.Top:
            stop = {i: 8, j: 16}
            indexPair = (i: number, j: number) => {return {from: {i: i, j: j}, to: {i: 15 - i, j: j}}}
            break;
        case Half.Left:
            stop = {i: 16, j: 8}
            indexPair = (i: number, j: number) => {return {from: {i: i, j: j}, to: {i: i, j: 15 - j}}}
            break;
        case Half.Bottom:
            stop = {i: 8, j: 16}
            indexPair = (i: number, j: number) => {return {from: {i: 15 - i, j: j}, to: {i: i, j: j}}}
            break;
        case Half.Right:
            stop = {i: 16, j: 8}
            indexPair = (i: number, j: number) => {return {from: {i: i, j: 15 - j}, to: {i: i, j: j}}}
            break;
    }
    console.log(stop, indexPair)
